...
import thermalData from "../../formData/thermalData"
...
import { backBtn, resetBtn, saveBtn } from '../../formData/bottomNavData'
import { useNavigate } from 'react-router-dom'
import { BuildingContext } from '../../components/Context/context'

export default function ThermalInfo(props) {
    ...
    for (const [key, value] of Object.entries(props.project)) {
        ...
        let title = thermalData.find(item => item.id === key)
        if (title != undefined) {
            combined_data.push({
                title: title.title,
                id: key,
        ...
        <>
            <div className="bg-white drop-shadow-md rounded p-5 my-5 h-fit">
                <div >
                    ...
                    <UploadFile building_name={building_name} data={thermalData} sub_section={subsection.toLowerCase().split(' ')[0]} />
