...
import projectInfoData from "../../formData/projectInfoData"
...
export default function ProjectInfo(props) {
    ...
    for (const [key, value] of Object.entries(props.project)) {
        ...
        let title = projectInfoData.find(item => item.id === key)
        if (title != undefined) {
            combined_data.push({
                title: title.title,
                id: key,
        ...
        <>
            <div className="bg-white drop-shadow-md rounded p-5 my-5 h-fit">
                <div >
                    ...
                    <UploadFile building_name={building_name} data={projectInfoData} sub_section={subsection.toLowerCase().split(' ')[0]} />
