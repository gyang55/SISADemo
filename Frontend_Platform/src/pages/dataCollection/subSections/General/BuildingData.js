...
import buildingData from "../../formData/buildingData"
...
export default function BuildingData(props) {
    ...
    for (const [key, value] of Object.entries(props.project)) {
        ...
        let title = buildingData.find(item => item.id === key)
        if (title != undefined) {
            combined_data.push({
                title: title.title,
                id: key,
        ...
        <>
            <div className="bg-white drop-shadow-md rounded p-5 my-5 h-fit">
                <div >
                    ...
                    <UploadFile building_name={building_name} data={buildingData} sub_section={subsection.toLowerCase().split(' ')[0]} />
