...
import facilityInfoData from "../../formData/facilityInfoData"
...
export default function FacilityInfo(props) {
    ...
    for (const [key, value] of Object.entries(props.project)) {
        ...
        let title = facilityInfoData.find(item => item.id === key)
        if (title != undefined) {
            combined_data.push({
                title: title.title,
                id: key,
        ...
        <>
            <div className="bg-white drop-shadow-md rounded p-5 my-5 h-fit">
                <div >
                    ...
                    <UploadFile building_name={building_name} data={facilityInfoData} sub_section={subsection.toLowerCase().split(' ')[0]} />
